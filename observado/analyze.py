...
def _train():
    x_full, y_full, z_full = _load_data(cqt)
    ...
    quality_clf = MLPClassifier(max_iter=1000)
    quality_clf.fit(x_train, z_train)
    print(accuracy_score(z_test, quality_clf.predict(x_test)))
    joblib.dump(quality_clf, os.path.join(dirname, '../data/models/quality.joblib'))


def analyze_root(chromagram: np.ndarray):
    if not os.path.exists(os.path.join(dirname, '../data/models/root.joblib')):
        _train()

    root_clf = joblib.load(os.path.join(dirname, '../data/models/root.joblib'))
    root_prob = root_clf.predict(chromagram)
    return [(_get_root(root_prob[i])) for i in range(len(root_prob))]


def analyze_chords(chromagram: np.ndarray, root_prob: list):
    if not os.path.exists(os.path.join(dirname, '../data/models/quality.joblib')):
        _train()

    quality_clf = joblib.load(os.path.join(dirname, '../data/models/quality.joblib'))
    assert len(chromagram) == len(root_prob)
    for i in range(len(chromagram)):
        chromagram[i] = np.roll(chromagram[i], -utils.note_values[root_prob[i]])
    quality_prob = quality_clf.predict(chromagram)

    assert len(root_prob) == len(quality_prob)
    return [(lambda a, b: 'N' if a == 'N' or b == 'N' else Chord(a + b))
            (root_prob[i], _get_quality(quality_prob[i])) for i in range(len(root_prob))]


def analyze(chromagram: np.ndarray) -> list:
    chromagram = chromagram.copy()
    return analyze_chords(chromagram, analyze_root(chromagram))
